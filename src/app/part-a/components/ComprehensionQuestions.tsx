...
interface PartAQuestionsProps {
    participantId: string;
    onComplete: () => void;
}

export default function ComprehensionQuestions({ participantId, onComplete }: PartAQuestionsProps) {
    ...
    useEffect(() => {
        fetch(getApiPath("/api/part-a/questions"))
            .then(res => res.json())
            .then(data => {
                setQuestions(data);
    ...
    const handleSubmit = async () => {
        ...
        const responseData = questions.map(q => ({
            ...
            reactionTimeMs: now - startTime
        }));

        try {
            await fetch(getApiPath("/api/part-a/submit-questions"), {
                method: "POST",
    ...
    return (
        <div className="max-w-5xl mx-auto glass-panel p-10 rounded-xl shadow-sm mt-12">
            <h2 className="text-2xl font-semibold mb-6 text-[var(--foreground)]">Comprehensive Questions</h2>
            <p className="mb-8 text-[var(--muted)] text-base">Please answer the following questions based on what you read.</p>
